...
try {
    ...
    Copy-Item -LiteralPath (Join-Path $projectPath 'app/build/outputs/apk/debug/app-debug.apk') -Destination (Join-Path $projectPath 'entrega/Voz-local-0.4.2.apk') -Force
    Get-FileHash -LiteralPath (Join-Path $projectPath 'entrega/Voz-local-0.4.2.apk') -Algorithm SHA256
