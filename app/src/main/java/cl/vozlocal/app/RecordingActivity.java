...
public class RecordingActivity extends Screen {
    ...
    @Override public void onCreate(Bundle state){
        ...
        if(!demo){ImageButton more=ui.iconButton(R.drawable.ic_more,"Más opciones",p.accent,0,44);more.setOnClickListener(v->RecordingActions.menu(this,recording,this::reload,this::releasePlayer));barActions.addView(more);}
        title=ui.heading(recording.title,Type.TITLE);title.setPadding(ui.dp(S1),ui.dp(S1),ui.dp(S1),ui.dp(S1));
        ...
        page.addView(title);
        LinearLayout metaRow=ui.row();metaRow.setPadding(ui.dp(S1),0,0,ui.dp(S4));meta=ui.text("",Type.SUBHEAD,p.muted);metaRow.addView(meta,new LinearLayout.LayoutParams(0,-2,1));chip=ui.chip("",p.muted,p.fill);metaRow.addView(chip);page.addView(metaRow,Ui.fill());
    ...
    private void reload(){
        ...
        chip.setText(demo?"Ejemplo":state.label);chip.setTextColor(demo?p.warning:state.fg(p));chip.setBackground(shape(this,demo?p.warningSoft:state.bg(p),99));
    ...
    private LinearLayout buildPlayer(){
        ...
        seek=new SeekBar(this);seek.setContentDescription("Posición de reproducción");seek.setProgressTintList(ColorStateList.valueOf(p.accent));seek.setThumbTintList(ColorStateList.valueOf(p.accent));seek.setProgressBackgroundTintList(ColorStateList.valueOf(p.faint));seek.setPadding(ui.dp(S2),ui.dp(S3),ui.dp(S2),ui.dp(S3));seek.setMax(Math.max(1,(int)recording.duration));
        card.addView(seek,Ui.fill());
        LinearLayout times=ui.row();times.setPadding(ui.dp(S2),0,ui.dp(S2),0);elapsed=ui.text("00:00",Type.FOOTNOTE,p.muted);elapsed.setFontFeatureSettings("tnum");remaining=ui.text("-"+Recording.time(recording.duration),Type.FOOTNOTE,p.muted);remaining.setFontFeatureSettings("tnum");times.addView(elapsed);times.addView(ui.flex());times.addView(remaining);card.addView(times,Ui.fill());
        LinearLayout controls=ui.row();controls.setGravity(Gravity.CENTER);controls.setPadding(0,ui.dp(S2),0,0);
        speed=ui.chip("1×",p.ink,p.fill);speed.setMinWidth(ui.dp(52));speed.setMinHeight(ui.dp(36));speed.setContentDescription("Velocidad 1x");speed.setAccessibilityDelegate(Ui.buttonRole());speed.setOnClickListener(v->cycleSpeed());controls.addView(speed);controls.addView(ui.flex());
        ImageButton back=ui.iconButton(R.drawable.ic_replay,"Retroceder 15 segundos",p.ink,0,52);back.setOnClickListener(v->skip(-15000));controls.addView(back);controls.addView(ui.space(S3));
        play=ui.iconButton(R.drawable.ic_play,"Reproducir",p.onPrimary,p.primaryFill,64);play.setOnClickListener(v->toggle());controls.addView(play);controls.addView(ui.space(S3));
        ImageButton fwd=ui.iconButton(R.drawable.ic_replay,"Adelantar 15 segundos",p.ink,0,52);fwd.setScaleX(-1f);fwd.setOnClickListener(v->skip(15000));controls.addView(fwd);
    ...
    private void showCallToAction(){
        ...
        card.addView(ui.tile(R.drawable.ic_sparkle,p.accent,p.accentSoft,44,24));
        TextView h=ui.text("Transcribe este audio",Type.TITLE_SMALL,p.ink);h.setPadding(0,ui.dp(S3),0,ui.dp(S1));card.addView(h);
        card.addView(ui.text(settings.hasKey()?"Se enviará a "+SettingsActivity.modelSummary(settings)+". Suele tardar menos que la duración del audio.":"Agrega tu clave de API para transcribir. Solo pagas lo que usas en tu cuenta del proveedor.",Type.SUBHEAD,p.muted));
    ...
    private void showQueued(RecState state){
        LinearLayout card=ui.card();LinearLayout row=ui.row();ProgressBar spin=new ProgressBar(this,null,android.R.attr.progressBarStyleSmall);spin.setIndeterminateTintList(ColorStateList.valueOf(p.accent));row.addView(spin,new LinearLayout.LayoutParams(ui.dp(24),ui.dp(24)));row.addView(ui.space(S3));
        LinearLayout texts=ui.column();texts.addView(ui.text("Transcripción en curso",Type.HEADLINE,p.ink));TextView d=ui.text(state.detail,Type.SUBHEAD,p.muted);d.setAccessibilityLiveRegion(View.ACCESSIBILITY_LIVE_REGION_POLITE);texts.addView(d);row.addView(texts,new LinearLayout.LayoutParams(0,-2,1));card.addView(row,Ui.fill());
        Settings s=new Settings(this);TextView note=ui.text("Puedes salir de la app: te avisaremos con una notificación."+(s.wifiOnly()?" Espera una red Wi-Fi.":"")+(s.charging()?" Espera a que el teléfono esté cargando.":""),Type.FOOTNOTE,p.muted);note.setPadding(0,ui.dp(S3),0,0);card.addView(note);
    ...
    private void showFailed(RecState state){
        LinearLayout card=ui.card();card.setBackground(shape(this,p.dangerSoft,R_CARD));LinearLayout row=ui.row();row.setGravity(Gravity.TOP);row.addView(ui.icon(R.drawable.ic_alert,p.danger,24));row.addView(ui.space(S3));
        LinearLayout texts=ui.column();texts.addView(ui.text("No se pudo transcribir",Type.HEADLINE,p.ink));texts.addView(ui.text(state.detail,Type.SUBHEAD,p.ink));row.addView(texts,new LinearLayout.LayoutParams(0,-2,1));card.addView(row,Ui.fill());
    ...
    private void showTranscript(){
        try{
            ...
            LinearLayout head=ui.row();head.setPadding(ui.dp(S1),ui.dp(S6),0,ui.dp(S2));head.addView(ui.heading("Transcripción",Type.HEADLINE));head.addView(ui.flex());
            ...
            if(diarized&&names.size()>0){
                ...
                for(String key:order){int color=p.speaker(order.indexOf(key));LinearLayout c=ui.row();c.setBackground(ui.ripple(shape(this,p.surface,99),99));c.setPadding(ui.dp(S3),ui.dp(6),ui.dp(14),ui.dp(6));c.setMinimumHeight(ui.dp(36));View dot=new View(this);dot.setBackground(oval(color));c.addView(dot,new LinearLayout.LayoutParams(ui.dp(10),ui.dp(10)));c.addView(ui.space(S2));c.addView(ui.text(names.get(key),Type.SUBHEAD,p.ink));c.setClickable(true);c.setContentDescription("Cambiar nombre de "+names.get(key));c.setAccessibilityDelegate(Ui.buttonRole());c.setOnClickListener(v->editSpeakers());LinearLayout.LayoutParams lp=Ui.wrap();lp.setMarginEnd(ui.dp(S2));chips.addView(c,lp);}
            ...
            if(segments.length()==0)card.addView(ui.text("No se detectó habla en este audio.",Type.BODY,p.muted));
            String last=null;
            for(int i=0;i<segments.length();i++){
                ...
                if(!speaker.equals(last)){
                    ...
                    if(diarized){TextView name=ui.text(names.get(speaker),Type.SUBHEAD,p.speaker(order.indexOf(speaker)));name.setTypeface(typeface(Weight.MEDIUM));h.addView(name);h.addView(ui.space(S2));}
                    TextView time=ui.text(Recording.time((long)(start*1000)),Type.FOOTNOTE,p.muted);time.setFontFeatureSettings("tnum");
                    if(!demo){time.setTextColor(p.accent);time.setPadding(ui.dp(S1),ui.dp(S1),ui.dp(S1),ui.dp(S1));time.setBackground(ui.ripple(null,R_SMALL));time.setContentDescription("Escuchar desde "+Recording.time((long)(start*1000)));time.setAccessibilityDelegate(Ui.buttonRole());time.setOnClickListener(v->seekTo(start));}
                    h.addView(time);card.addView(h,Ui.fill());last=speaker;
                }else card.addView(ui.space(S2));
                TextView text=ui.text(s.getString("text").trim(),Type.BODY,p.ink);text.setTextIsSelectable(true);text.setLineSpacing(ui.dp(4),1f);card.addView(text,Ui.fill());
            }
            ...
            LinearLayout bar=ui.row();bar.setBackground(shape(this,p.surface,R_CARD));bar.setPadding(ui.dp(S1),ui.dp(S1),ui.dp(S1),ui.dp(S1));
        ...
        }catch(Exception e){content.addView(ui.text("No se pudo leer la transcripción.",Type.BODY,p.danger));}
    }
    private View note(int icon,String value){LinearLayout n=ui.row();n.setGravity(Gravity.TOP);n.setBackground(shape(this,p.accentSoft,R_CONTROL));n.setPadding(ui.dp(S3),ui.dp(S3),ui.dp(S3),ui.dp(S3));n.addView(ui.icon(icon,p.accent,18));n.addView(ui.space(S2));n.addView(ui.text(value,Type.FOOTNOTE,p.ink),new LinearLayout.LayoutParams(0,-2,1));LinearLayout.LayoutParams lp=Ui.fill();lp.bottomMargin=ui.dp(S3);n.setLayoutParams(lp);return n;}
