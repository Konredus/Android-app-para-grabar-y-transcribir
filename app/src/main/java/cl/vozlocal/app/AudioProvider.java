...
public class AudioProvider extends ContentProvider {
    ...
    private File resolve(Uri uri) throws FileNotFoundException {
        ...
        if (name == null || !name.matches("[a-f0-9-]{36}\\.m4a")) throw new FileNotFoundException();
        File f = new File(Recording.directory(getContext()), name);
    ...
    @Override public Cursor query(Uri uri, String[] projection, String selection, String[] args, String sort) {
        try {
            ...
            for (int i=0; i<cols.length; i++) { if (OpenableColumns.DISPLAY_NAME.equals(cols[i])) row[i] = f.getName(); else if (OpenableColumns.SIZE.equals(cols[i])) row[i] = f.length(); }
